...
func (s *Store) CreateDocument(ctx context.Context, namespace, filename, format, fileHash, filePath, instanceID string) (*Document, error) {
	doc := &Document{}
	err := s.pool.QueryRow(ctx, `
		INSERT INTO documents (namespace, filename, format, file_hash, file_path, instance_id)
		VALUES ($1, $2, $3, $4, $5, $6)
		RETURNING id, namespace, filename, format, status, error, chunk_count, file_hash, file_path, instance_id, created_at, updated_at
	`, namespace, filename, format, fileHash, filePath, instanceID).Scan(
		&doc.ID, &doc.Namespace, &doc.Filename, &doc.Format, &doc.Status,
		&doc.Error, &doc.ChunkCount, &doc.FileHash, &doc.FilePath, &doc.InstanceID, &doc.CreatedAt, &doc.UpdatedAt,
	)
	if err != nil {
...
func (s *Store) FindDocumentByHash(ctx context.Context, namespace, fileHash string) (*Document, error) {
	doc := &Document{}
	err := s.pool.QueryRow(ctx, `
		SELECT id, namespace, filename, format, status, error, chunk_count, file_hash, file_path, instance_id, created_at, updated_at
		FROM documents
...
// ListUnfinishedDocuments returns all documents that have a file_path set and
// are in pending or processing state. Used at startup to re-enqueue work that
// was interrupted by a restart.
func (s *Store) ListUnfinishedDocuments(ctx context.Context, instanceID string) ([]*Document, error) {
	rows, err := s.pool.Query(ctx, `
		SELECT id, namespace, filename, format, status, error, chunk_count, file_hash, file_path, instance_id, created_at, updated_at
		FROM documents
		WHERE status IN ('pending', 'processing') AND file_path IS NOT NULL AND instance_id = $1
		ORDER BY created_at ASC
	`, instanceID)
	if err != nil {
		return nil, fmt.Errorf("listing unfinished documents: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, fmt.Errorf("scanning unfinished document: %w", err)
		}
		docs = append(docs, doc)
	}
	return docs, nil
...
// ClearFilePath sets file_path to NULL for a document, indicating the file
// has been processed and can no longer be found on disk.
func (s *Store) ClearFilePath(ctx context.Context, id uuid.UUID) error {
	_, err := s.pool.Exec(ctx, `UPDATE documents SET file_path = NULL WHERE id = $1`, id)
	if err != nil {
		return fmt.Errorf("clearing file_path for document %s: %w", id, err)
	}
	return nil
}
