...
#   make build-mcp      Compile the MCP server binary
...
.PHONY: dependencies run build build-mcp test lint swagger \
...
dependencies:
	...
	# docs/api is a local package generated by swag. go mod download would try
	# to fetch it from GitHub if the directory doesn't exist yet, so we generate
	# swagger first and fetch modules after.
...
# swagger is an alias for swagger-gen, kept for convenience.
swagger: swagger-gen
	@true
...
MCP_DIR       := mcp
MCP_BINARY    := memex-mcp

## Compile the MCP server binary.
build-mcp:
	@echo "==> Compiling MCP binary for $(shell go env GOOS)/$(shell go env GOARCH)..."
	@mkdir -p $(DIST_DIR)
	cd $(MCP_DIR) && \
	  CGO_ENABLED=$(CGO_ENABLED) go build \
	    -ldflags="$(LDFLAGS)" \
	    -o ../$(DIST_DIR)/$(MCP_BINARY) \
	    ./cmd/
	@echo "==> MCP binary ready: $(DIST_DIR)/$(MCP_BINARY)"
...
help:
	...
	@echo "memex $(VERSION)"
	@echo ""
	@awk '/^##/{desc=substr($$0,4); next} /^[a-z][a-z_-]+:/{if (desc) printf "  %-20s %s\n", $$1, desc; desc=""}' \
	  $(MAKEFILE_LIST)
	@echo ""
